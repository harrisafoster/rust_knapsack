pub mod knapsack {
    ...
    fn check_all_combinations(
        ...
        if ((prices[index - 1] * 100.0) as usize) <= rmb {
            ...
            combination_matrix[index][rmb] = check_all_combinations(
                combination_matrix,
                prices,
                earnings,
                rmb,
    ...
    pub fn run_algo(capacity: i64, csv_data: &[u8]) -> Vec<Stock> {
        let stocks: Vec<Stock> = set_up_and_sort_data(csv_data);
        ...
        let price_col: Vec<f64> = stocks
            .iter()
            .map(|stock: &Stock| stock.price)
            .collect();
        let earnings_col: Vec<f64> = stocks
            .iter()
            .map(|stock: &Stock| stock.earnings)
            .collect();
        check_all_combinations(&mut combination_matrix, &price_col, &earnings_col, rmb, n_stocks);
